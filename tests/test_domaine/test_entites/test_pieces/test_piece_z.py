...
class TestPieceZ(unittest.TestCase):
    ...
    def test_piece_z_peut_etre_creee(self):
        ...
        # Arrange & Act
        piece = PieceZ.creer(x_pivot=5, y_pivot=0)
        
        # Assert - Form Z horizontal (orientation par défaut)
        # ██   ← positions (4,0) et (5,0)
        #  ██  ← positions (5,1) et (6,1)
        positions_attendues = [
            Position(4, 0),  # Gauche-haut
            Position(5, 0),  # Centre-haut (pivot)
    ...
    def test_piece_z_peut_tourner_horizontal_vers_vertical(self):
        ...
        # Arrange
        piece = PieceZ.creer(x_pivot=5, y_pivot=1)
        
        # Act
        piece.tourner()
        
        # Assert - Form Z vertical autour du pivot (5,1)
        #  █   ← position (6,0)
        # ██   ← positions (5,1) et (6,1)  
        # █    ← position (5,2)
        positions_attendues = [
            Position(6, 0),  # Haut-droite
    ...
    def test_piece_z_peut_tourner_vertical_vers_horizontal(self):
        ...
        positions_attendues = [
            Position(4, 1),  # Gauche-haut
            Position(5, 1),  # Centre-haut (pivot)
