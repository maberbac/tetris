...
class TestPieceO(unittest.TestCase):
    ...
    def test_piece_o_peut_etre_creee(self):
        ...
        piece = PieceO.creer(x_pivot=5, y_pivot=0)
        
        # Assert : Vérifier les propriétés
        self.assertEqual(piece.type_piece, TypePiece.O)
        self.assertEqual(len(piece.positions), 4)
        
        # Positions attendues : carré 2x2 centré sur x=5, y=0 (zone invisible)
        # ██
        # ██
        positions_attendues = [
            Position(5, -1), Position(6, -1),    # Ligne du haut (zone invisible)
            Position(5, 0), Position(6, 0)       # Ligne du bas
        ]
        self.assertEqual(piece.positions, positions_attendues)
        
        # Pivot au centre du carré (entre les 4 blocs)
        # Pour un carré 2x2, le pivot logique est (5.5, -0.5)  
    ...
    def test_piece_o_rotation_ne_fait_rien(self):
        ...
        """
        # Arrange : PieceO avec positions initiales (zone invisible)
    ...
    def test_piece_o_peut_se_deplacer(self):
        ...
        # Assert : Toutes positions déplacées de (2, 3) depuis zone invisible
        positions_attendues = [
            Position(7, 2), Position(8, 2),    # Ligne du haut  
            Position(7, 3), Position(8, 3)     # Ligne du bas
        ]
        self.assertEqual(piece.positions, positions_attendues)
        self.assertEqual(piece.position_pivot, Position(7, 2))
