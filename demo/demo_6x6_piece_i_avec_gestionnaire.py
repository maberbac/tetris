"""
Démonstration Plateau 6x6 avec Pièce I - Version avec Gestionnaire d'Événements

Cette version utilise le vrai système de commandes et gestionnaire d'événements
du projet Tetris, montrant l'intégration complète de l'architecture avec la pièce I.
...
class CommandeChuteRapideDemoI(Commande):
    """Commande de chute rapide modifiée pour la démo I - sans génération de nouvelle pièce."""
    
    def execute(self, moteur) -> bool:
        """Fait tomber la pièce I jusqu'en bas sans la remplacer."""
        if hasattr(moteur, 'chute_rapide_demo'):
            return moteur.chute_rapide_demo()
        return False


class CommandePlacerPieceDemoI(Commande):
    """Commande pour placer une pièce I définitivement sur le plateau."""
    
    def execute(self, moteur) -> bool:
        """Place la pièce I sur le plateau et génère une nouvelle pièce I."""
        moteur.placer_piece_definitivement()
        moteur.generer_nouvelle_piece()
        return True


class CommandeDeplacerGaucheDemoI(Commande):
    """Commande de déplacement gauche corrigée pour la démo I."""
    
    def execute(self, moteur) -> bool:
        """Déplace la pièce I vers la gauche si possible."""
        ...
        if plateau.peut_placer_piece(piece):
            print(f"⬅️ Gauche I → {piece.positions}")
            return True
        else:
...
class CommandeDeplacerDroiteDemoI(Commande):
    """Commande de déplacement droite corrigée pour la démo I."""
    
    def execute(self, moteur) -> bool:
        """Déplace la pièce I vers la droite si possible."""
        piece = moteur.obtenir_piece_active()
        plateau = moteur.obtenir_plateau()
        ...
        # Vérifier si le déplacement est valide
        if plateau.peut_placer_piece(piece):
            print(f"➡️ Droite I → {piece.positions}")
            return True
        else:
            # Annuler le déplacement (positions ET pivot)
            piece.positions = positions_originales
...
class CommandeTournerDemoI(Commande):
    """Commande de rotation corrigée pour la démo I."""
    
    def execute(self, moteur) -> bool:
        """Fait tourner la pièce I si possible."""
        piece = moteur.obtenir_piece_active()
        plateau = moteur.obtenir_plateau()
        ...
        if plateau.peut_placer_piece(piece):
            print(f"🔄 Rotation I → {piece.positions}")
            return True
        else:
            # Annuler la rotation
            piece.positions = positions_originales
...
class PlateauDemoI:
    """Plateau simple pour la démonstration 6x6 avec pièce I."""
...
class MoteurDemo6x6I:
    """
    Moteur simple pour la démonstration 6x6 avec pièce I.
    Implémente le protocole MoteurJeu pour fonctionner avec les commandes.
    """
    
    def __init__(self):
        # Plateau 6x6 pour la démo
        self.plateau = PlateauDemoI(largeur=6, hauteur=6)
        
        # Créer la pièce I au centre
        fabrique = FabriquePieces()
        self.piece_active = fabrique.creer(TypePiece.I, x_pivot=2, y_pivot=2)
        
        # État du jeu
        self.en_pause = False
        self.afficher_menu = False
        
        print(f"🎮 Moteur démo I 6x6 initialisé")
        print(f"📍 Pièce I créée: {self.piece_active.positions}")
    
    def obtenir_piece_active(self) -> Piece:
        """Retourne la pièce actuellement contrôlée."""
        return self.piece_active
    
    def obtenir_plateau(self) -> PlateauDemoI:
        """Retourne le plateau de jeu."""
        return self.plateau
    
    def faire_descendre_piece(self) -> bool:
        """Fait descendre la pièce I d'une ligne si possible."""
        ...
        if self.plateau.peut_placer_piece(self.piece_active):
            print(f"⬇️ Descente I → {self.piece_active.positions}")
            return True
        else:
    ...
    def placer_piece_definitivement(self) -> None:
        """Place la pièce I définitivement sur le plateau."""
        # Placer la pièce sur le plateau
        self.plateau.placer_piece(self.piece_active)
        print(f"📍 Pièce I placée définitivement à: {self.piece_active.positions}")
    
    def generer_nouvelle_piece(self) -> None:
        """Génère une nouvelle pièce I (pour cette démo, on recrée une I)."""
        fabrique = FabriquePieces()
        self.piece_active = fabrique.creer(TypePiece.I, x_pivot=2, y_pivot=1)
        print(f"✨ Nouvelle pièce I générée: {self.piece_active.positions}")
    
    def chute_rapide_demo(self) -> bool:
        """Chute rapide modifiée pour la démo I - sans génération de nouvelle pièce."""
        ...
        if nb_lignes_descendues > 0:
            print(f"🚀 Chute rapide I: {nb_lignes_descendues} lignes → {self.piece_active.positions}")
...
class AffichageAvecGestionnaireI:
    """Affichage pour la démo I avec gestionnaire d'événements."""
    ...
    def __init__(self):
        ...
        pygame.display.set_caption("Demo 6x6 - Pièce I avec Gestionnaire d'Événements")
        
        # Couleurs
        self.noir = (0, 0, 0)
        self.gris = (64, 64, 64)
        self.blanc = (255, 255, 255)
        self.cyan = (0, 255, 255)      # Cyan pour pièce I (couleur traditionnelle)
    ...
    def dessiner(self, moteur: MoteurDemo6x6I):
        ...
        # Pièces du plateau (si il y en a) - en jaune pour les distinguer de la pièce I active
        for position in moteur.plateau.obtenir_positions_occupees():
            ...
            rect = pygame.Rect(x, y, self.taille_cellule, self.taille_cellule)
            pygame.draw.rect(self.ecran, (255, 255, 0), rect)  # Jaune pour pièces placées
        ...
        for pos in moteur.piece_active.positions:
            ...
            rect = pygame.Rect(x, y, self.taille_cellule, self.taille_cellule)
            pygame.draw.rect(self.ecran, self.cyan, rect)
            pygame.draw.rect(self.ecran, self.blanc, rect, 2)
        ...
        # Titre
        titre = "Demo 6x6 - Pièce I avec Gestionnaire d'Événements"
        ...
        instructions = [
            ...
            "ESPACE : Chute rapide | ENTRÉE : Placer pièce I",
...
class GestionnaireDemoPersonnaliseI(GestionnaireEvenements):
    """Gestionnaire d'événements personnalisé pour la démo I."""
    
    def _creer_commandes(self):
        """Crée le mapping avec nos commandes personnalisées pour la pièce I."""
        from src.domaine.services.commandes import (
            CommandeDescendre, CommandePause, CommandeAfficherMenu
        )
        
        return {
            ToucheClavier.GAUCHE: CommandeDeplacerGaucheDemoI(),           # Notre version corrigée pour I
            ToucheClavier.DROITE: CommandeDeplacerDroiteDemoI(),          # Notre version corrigée pour I
            ToucheClavier.ROTATION: CommandeTournerDemoI(),               # Notre version corrigée pour I
            ToucheClavier.CHUTE_RAPIDE: CommandeDescendre(),
            ToucheClavier.CHUTE_INSTANTANEE: CommandeChuteRapideDemoI(),  # Notre version modifiée pour I
            ToucheClavier.MENU: CommandeAfficherMenu(),
            ToucheClavier.PAUSE: CommandePause(),
        }


class DemoAvecGestionnaireI:
    """Démonstration principale utilisant le gestionnaire d'événements pour la pièce I."""
    
    def __init__(self):
        self.moteur = MoteurDemo6x6I()
        self.affichage = AffichageAvecGestionnaireI()
        self.gestionnaire = GestionnaireDemoPersonnaliseI()  # Notre gestionnaire personnalisé
        ...
        print("🎮 Démonstration I avec gestionnaire d'événements initialisée")
    ...
    def executer(self):
        ...
        print("\n=== DEMO 6x6 PIÈCE I AVEC GESTIONNAIRE D'ÉVÉNEMENTS ===")
            ...
            for event in pygame.event.get():
                ...
                # Touches spéciales pour la démo
                    ...
                    if event.key == pygame.K_RETURN:  # Entrée pour placer la pièce
                        ...
                        print("⏎ Pièce I placée manuellement avec Entrée")
        ...
        print("Démonstration I terminée.")


if __name__ == "__main__":
    demo = DemoAvecGestionnaireI()
    demo.executer()
