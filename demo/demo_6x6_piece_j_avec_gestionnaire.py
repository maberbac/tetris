"""
Démonstration Plateau 6x6 avec Pièce J - Version avec Gestionnaire d'Événements

Cette version utilise le vrai système de commandes et gestionnaire d'événements
du projet Tetris, montrant l'intégration complète de l'architecture avec la pièce J.
...
class CommandeChuteRapideDemoJ(Commande):
    """Commande de chute rapide modifiée pour la démo J - sans génération de nouvelle pièce."""
    
    def execute(self, moteur) -> bool:
        """Fait tomber la pièce J jusqu'en bas sans la remplacer."""
        if hasattr(moteur, 'chute_rapide_demo'):
            return moteur.chute_rapide_demo()
        return False


class CommandePlacerPieceDemoJ(Commande):
    """Commande pour placer une pièce J définitivement sur le plateau."""
    
    def execute(self, moteur) -> bool:
        """Place la pièce J sur le plateau et génère une nouvelle pièce J."""
        moteur.placer_piece_definitivement()
        moteur.generer_nouvelle_piece()
        return True


class CommandeDeplacerGaucheDemoJ(Commande):
    """Commande de déplacement gauche corrigée pour la démo J."""
    
    def execute(self, moteur) -> bool:
        """Déplace la pièce J vers la gauche si possible."""
        ...
        if plateau.peut_placer_piece(piece):
            print(f"⬅️ Gauche J → {piece.positions}")
            return True
        else:
...
class CommandeDeplacerDroiteDemoJ(Commande):
    """Commande de déplacement droite corrigée pour la démo J."""
    
    def execute(self, moteur) -> bool:
        """Déplace la pièce J vers la droite si possible."""
        piece = moteur.obtenir_piece_active()
        plateau = moteur.obtenir_plateau()
        ...
        # Vérifier si le déplacement est valide
        if plateau.peut_placer_piece(piece):
            print(f"➡️ Droite J → {piece.positions}")
            return True
        else:
            # Annuler le déplacement (positions ET pivot)
            piece.positions = positions_originales
...
class CommandeTournerDemoJ(Commande):
    """Commande de rotation corrigée pour la démo J."""
    
    def execute(self, moteur) -> bool:
        """Fait tourner la pièce J si possible."""
        piece = moteur.obtenir_piece_active()
        plateau = moteur.obtenir_plateau()
        ...
        if plateau.peut_placer_piece(piece):
            print(f"🔄 Rotation J → {piece.positions}")
            return True
        else:
            # Annuler la rotation
            piece.positions = positions_originales
...
class PlateauDemoJ:
    """Plateau simple pour la démonstration 6x6 avec pièce J."""
...
class MoteurDemo6x6J:
    """
    Moteur simple pour la démonstration 6x6 avec pièce J.
    Implémente le protocole MoteurJeu pour fonctionner avec les commandes.
    """
    
    def __init__(self):
        # Plateau 6x6 pour la démo
        self.plateau = PlateauDemoJ(largeur=6, hauteur=6)
        
        # Créer la pièce J au centre
        fabrique = FabriquePieces()
        self.piece_active = fabrique.creer(TypePiece.J, x_pivot=2, y_pivot=2)
        
        # État du jeu
        self.en_pause = False
        self.afficher_menu = False
        
        print(f"🎮 Moteur démo J 6x6 initialisé")
        print(f"📍 Pièce J créée: {self.piece_active.positions}")
    
    def obtenir_piece_active(self) -> Piece:
        """Retourne la pièce actuellement contrôlée."""
        return self.piece_active
    
    def obtenir_plateau(self) -> PlateauDemoJ:
        """Retourne le plateau de jeu."""
        return self.plateau
    
    def faire_descendre_piece(self) -> bool:
        """Fait descendre la pièce J d'une ligne si possible."""
        ...
        if self.plateau.peut_placer_piece(self.piece_active):
            print(f"⬇️ Descente J → {self.piece_active.positions}")
            return True
        else:
    ...
    def placer_piece_definitivement(self) -> None:
        """Place la pièce J définitivement sur le plateau."""
        # Placer la pièce sur le plateau
        self.plateau.placer_piece(self.piece_active)
        print(f"📍 Pièce J placée définitivement à: {self.piece_active.positions}")
    
    def generer_nouvelle_piece(self) -> None:
        """Génère une nouvelle pièce J (pour cette démo, on recrée une J)."""
        fabrique = FabriquePieces()
        self.piece_active = fabrique.creer(TypePiece.J, x_pivot=2, y_pivot=1)
        print(f"✨ Nouvelle pièce J générée: {self.piece_active.positions}")
    
    def chute_rapide_demo(self) -> bool:
        """Chute rapide modifiée pour la démo J - sans génération de nouvelle pièce."""
        ...
        if nb_lignes_descendues > 0:
            print(f"🚀 Chute rapide J: {nb_lignes_descendues} lignes → {self.piece_active.positions}")
...
class AffichageAvecGestionnaireJ:
    """Affichage pour la démo J avec gestionnaire d'événements."""
    ...
    def __init__(self):
        ...
        pygame.display.set_caption("Demo 6x6 - Pièce J avec Gestionnaire d'Événements")
        
        # Couleurs
        self.noir = (0, 0, 0)
        self.gris = (64, 64, 64)
        self.blanc = (255, 255, 255)
        self.bleu = (0, 0, 255)       # Bleu pour pièce J
    ...
    def dessiner(self, moteur: MoteurDemo6x6J):
        ...
        # Titre
        titre = "Demo 6x6 - Pièce J avec Gestionnaire d'Événements"
        ...
        instructions = [
            ...
            "ESPACE : Chute rapide | ENTRÉE : Placer pièce J",
...
class GestionnaireDemoPersonnaliseJ(GestionnaireEvenements):
    """Gestionnaire d'événements personnalisé pour la démo J."""
    
    def _creer_commandes(self):
        """Crée le mapping avec nos commandes personnalisées pour la pièce J."""
        from src.domaine.services.commandes import (
            CommandeDescendre, CommandePause, CommandeAfficherMenu
        )
        
        return {
            ToucheClavier.GAUCHE: CommandeDeplacerGaucheDemoJ(),           # Notre version corrigée pour J
            ToucheClavier.DROITE: CommandeDeplacerDroiteDemoJ(),          # Notre version corrigée pour J
            ToucheClavier.ROTATION: CommandeTournerDemoJ(),               # Notre version corrigée pour J
            ToucheClavier.CHUTE_RAPIDE: CommandeDescendre(),
            ToucheClavier.CHUTE_INSTANTANEE: CommandeChuteRapideDemoJ(),  # Notre version modifiée pour J
            ToucheClavier.MENU: CommandeAfficherMenu(),
            ToucheClavier.PAUSE: CommandePause(),
        }


class DemoAvecGestionnaireJ:
    """Démonstration principale utilisant le gestionnaire d'événements pour la pièce J."""
    
    def __init__(self):
        self.moteur = MoteurDemo6x6J()
        self.affichage = AffichageAvecGestionnaireJ()
        self.gestionnaire = GestionnaireDemoPersonnaliseJ()  # Notre gestionnaire personnalisé
        ...
        print("🎮 Démonstration J avec gestionnaire d'événements initialisée")
    ...
    def executer(self):
        ...
        print("\n=== DEMO 6x6 PIÈCE J AVEC GESTIONNAIRE D'ÉVÉNEMENTS ===")
            ...
            for event in pygame.event.get():
                ...
                # Touches spéciales pour la démo
                    ...
                    if event.key == pygame.K_RETURN:  # Entrée pour placer la pièce
                        ...
                        print("⏎ Pièce J placée manuellement avec Entrée")
        ...
        print("Démonstration J terminée.")


if __name__ == "__main__":
    demo = DemoAvecGestionnaireJ()
    demo.executer()
