"""
Démonstration Plateau 6x6 avec Pièce O - Version avec Gestionnaire d'Événements

Cette version utilise le vrai système de commandes et gestionnaire d'événements
du projet Tetris, montrant l'intégration complète de l'architecture avec la pièce O.
...
class CommandeChuteRapideDemoO(Commande):
    """Commande de chute rapide modifiée pour la démo O - sans génération de nouvelle pièce."""
    
    def execute(self, moteur) -> bool:
        """Fait tomber la pièce O jusqu'en bas sans la remplacer."""
        if hasattr(moteur, 'chute_rapide_demo'):
            return moteur.chute_rapide_demo()
        return False


class CommandePlacerPieceDemoO(Commande):
    """Commande pour placer une pièce O définitivement sur le plateau."""
    
    def execute(self, moteur) -> bool:
        """Place la pièce O sur le plateau et génère une nouvelle pièce O."""
        moteur.placer_piece_definitivement()
        moteur.generer_nouvelle_piece()
        return True


class CommandeDeplacerGaucheDemoO(Commande):
    """Commande de déplacement gauche corrigée pour la démo O."""
    
    def execute(self, moteur) -> bool:
        """Déplace la pièce O vers la gauche si possible."""
        ...
        if plateau.peut_placer_piece(piece):
            print(f"⬅️ Gauche O → {piece.positions}")
            return True
        else:
...
class CommandeDeplacerDroiteDemoO(Commande):
    """Commande de déplacement droite corrigée pour la démo O."""
    
    def execute(self, moteur) -> bool:
        """Déplace la pièce O vers la droite si possible."""
        piece = moteur.obtenir_piece_active()
        plateau = moteur.obtenir_plateau()
        ...
        # Vérifier si le déplacement est valide
        if plateau.peut_placer_piece(piece):
            print(f"➡️ Droite O → {piece.positions}")
            return True
        else:
            # Annuler le déplacement (positions ET pivot)
            piece.positions = positions_originales
...
class CommandeTournerDemoO(Commande):
    """Commande de rotation corrigée pour la démo O."""
    
    def execute(self, moteur) -> bool:
        """Fait tourner la pièce O si possible (la pièce O ne change pas d'apparence)."""
        piece = moteur.obtenir_piece_active()
        plateau = moteur.obtenir_plateau()
        ...
        if plateau.peut_placer_piece(piece):
            print(f"🔄 Rotation O → {piece.positions} (apparence inchangée)")
            return True
        else:
            # Annuler la rotation
            piece.positions = positions_originales
...
class PlateauDemoO:
    """Plateau simple pour la démonstration 6x6 avec pièce O."""
...
class MoteurDemo6x6O:
    """
    Moteur simple pour la démonstration 6x6 avec pièce O.
    Implémente le protocole MoteurJeu pour fonctionner avec les commandes.
    """
    
    def __init__(self):
        # Plateau 6x6 pour la démo
        self.plateau = PlateauDemoO(largeur=6, hauteur=6)
        
        # Créer la pièce O au centre
        fabrique = FabriquePieces()
        self.piece_active = fabrique.creer(TypePiece.O, x_pivot=2, y_pivot=2)
        
        # État du jeu
        self.en_pause = False
        self.afficher_menu = False
        
        print(f"🎮 Moteur démo O 6x6 initialisé")
        print(f"📍 Pièce O créée: {self.piece_active.positions}")
    
    def obtenir_piece_active(self) -> Piece:
        """Retourne la pièce actuellement contrôlée."""
        return self.piece_active
    
    def obtenir_plateau(self) -> PlateauDemoO:
        """Retourne le plateau de jeu."""
        return self.plateau
    
    def faire_descendre_piece(self) -> bool:
        """Fait descendre la pièce O d'une ligne si possible."""
        ...
        if self.plateau.peut_placer_piece(self.piece_active):
            print(f"⬇️ Descente O → {self.piece_active.positions}")
            return True
        else:
    ...
    def placer_piece_definitivement(self) -> None:
        """Place la pièce O définitivement sur le plateau."""
        # Placer la pièce sur le plateau
        self.plateau.placer_piece(self.piece_active)
        print(f"📍 Pièce O placée définitivement à: {self.piece_active.positions}")
    
    def generer_nouvelle_piece(self) -> None:
        """Génère une nouvelle pièce O (pour cette démo, on recrée une O)."""
        fabrique = FabriquePieces()
        self.piece_active = fabrique.creer(TypePiece.O, x_pivot=2, y_pivot=1)
        print(f"✨ Nouvelle pièce O générée: {self.piece_active.positions}")
    
    def chute_rapide_demo(self) -> bool:
        """Chute rapide modifiée pour la démo O - sans génération de nouvelle pièce."""
        ...
        if nb_lignes_descendues > 0:
            print(f"🚀 Chute rapide O: {nb_lignes_descendues} lignes → {self.piece_active.positions}")
...
class AffichageAvecGestionnaireO:
    """Affichage pour la démo O avec gestionnaire d'événements."""
    ...
    def __init__(self):
        ...
        pygame.display.set_caption("Demo 6x6 - Pièce O avec Gestionnaire d'Événements")
        
        # Couleurs
        self.noir = (0, 0, 0)
        self.gris = (64, 64, 64)
        self.blanc = (255, 255, 255)
        self.jaune = (255, 255, 0)      # Jaune pour pièce O (couleur traditionnelle)
    ...
    def dessiner(self, moteur: MoteurDemo6x6O):
        ...
        # Pièces du plateau (si il y en a) - en orange pour les distinguer de la pièce O active
        for position in moteur.plateau.obtenir_positions_occupees():
            ...
            rect = pygame.Rect(x, y, self.taille_cellule, self.taille_cellule)
            pygame.draw.rect(self.ecran, (255, 165, 0), rect)  # Orange pour pièces placées
        ...
        for pos in moteur.piece_active.positions:
            ...
            rect = pygame.Rect(x, y, self.taille_cellule, self.taille_cellule)
            pygame.draw.rect(self.ecran, self.jaune, rect)
            pygame.draw.rect(self.ecran, self.blanc, rect, 2)
        ...
        # Titre
        titre = "Demo 6x6 - Pièce O avec Gestionnaire d'Événements"
        ...
        # Contrôles
        instructions = [
            "← → : Déplacer | ↑ : Rotation (pas d'effet visuel) | ↓ : Descendre",
            "ESPACE : Chute rapide | ENTRÉE : Placer pièce O",
...
class GestionnaireDemoPersonnaliseO(GestionnaireEvenements):
    """Gestionnaire d'événements personnalisé pour la démo O."""
    
    def _creer_commandes(self):
        """Crée le mapping avec nos commandes personnalisées pour la pièce O."""
        from src.domaine.services.commandes import (
            CommandeDescendre, CommandePause, CommandeAfficherMenu
        )
        
        return {
            ToucheClavier.GAUCHE: CommandeDeplacerGaucheDemoO(),           # Notre version corrigée pour O
            ToucheClavier.DROITE: CommandeDeplacerDroiteDemoO(),          # Notre version corrigée pour O
            ToucheClavier.ROTATION: CommandeTournerDemoO(),               # Notre version corrigée pour O
            ToucheClavier.CHUTE_RAPIDE: CommandeDescendre(),
            ToucheClavier.CHUTE_INSTANTANEE: CommandeChuteRapideDemoO(),  # Notre version modifiée pour O
            ToucheClavier.MENU: CommandeAfficherMenu(),
            ToucheClavier.PAUSE: CommandePause(),
        }


class DemoAvecGestionnaireO:
    """Démonstration principale utilisant le gestionnaire d'événements pour la pièce O."""
    
    def __init__(self):
        self.moteur = MoteurDemo6x6O()
        self.affichage = AffichageAvecGestionnaireO()
        self.gestionnaire = GestionnaireDemoPersonnaliseO()  # Notre gestionnaire personnalisé
        ...
        print("🎮 Démonstration O avec gestionnaire d'événements initialisée")
    ...
    def executer(self):
        ...
        print("\n=== DEMO 6x6 PIÈCE O AVEC GESTIONNAIRE D'ÉVÉNEMENTS ===")
        print("Contrôles via gestionnaire:")
        print("  ← → : Déplacer (avec répétition)")
        print("  ↑ : Rotation (pas d'effet visuel - pièce O est symétrique)")
            ...
            for event in pygame.event.get():
                ...
                # Touches spéciales pour la démo
                    ...
                    if event.key == pygame.K_RETURN:  # Entrée pour placer la pièce
                        ...
                        print("⏎ Pièce O placée manuellement avec Entrée")
        ...
        print("Démonstration O terminée.")


if __name__ == "__main__":
    demo = DemoAvecGestionnaireO()
    demo.executer()
