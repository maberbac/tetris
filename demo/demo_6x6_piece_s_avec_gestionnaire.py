"""
Démonstration Plateau 6x6 avec Pièce S - Version avec Gestionnaire d'Événements

Cette version utilise le vrai système de commandes et gestionnaire d'événements
du projet Tetris, montrant l'intégration complète de l'architecture avec la pièce S.
...
class CommandeChuteRapideDemoS(Commande):
    """Commande de chute rapide modifiée pour la démo S - sans génération de nouvelle pièce."""
    
    def execute(self, moteur) -> bool:
        """Fait tomber la pièce S jusqu'en bas sans la remplacer."""
        if hasattr(moteur, 'chute_rapide_demo'):
            return moteur.chute_rapide_demo()
        return False


class CommandePlacerPieceDemoS(Commande):
    """Commande pour placer une pièce S définitivement sur le plateau."""
    
    def execute(self, moteur) -> bool:
        """Place la pièce S sur le plateau et génère une nouvelle pièce S."""
        moteur.placer_piece_definitivement()
        moteur.generer_nouvelle_piece()
        return True


class CommandeDeplacerGaucheDemoS(Commande):
    """Commande de déplacement gauche corrigée pour la démo S."""
    
    def execute(self, moteur) -> bool:
        """Déplace la pièce S vers la gauche si possible."""
        ...
        if plateau.peut_placer_piece(piece):
            print(f"⬅️ Gauche S → {piece.positions}")
            return True
        else:
...
class CommandeDeplacerDroiteDemoS(Commande):
    """Commande de déplacement droite corrigée pour la démo S."""
    
    def execute(self, moteur) -> bool:
        """Déplace la pièce S vers la droite si possible."""
        piece = moteur.obtenir_piece_active()
        plateau = moteur.obtenir_plateau()
        ...
        # Vérifier si le déplacement est valide
        if plateau.peut_placer_piece(piece):
            print(f"➡️ Droite S → {piece.positions}")
            return True
        else:
            # Annuler le déplacement (positions ET pivot)
            piece.positions = positions_originales
...
class CommandeTournerDemoS(Commande):
    """Commande de rotation corrigée pour la démo S."""
    
    def execute(self, moteur) -> bool:
        """Fait tourner la pièce S si possible."""
        piece = moteur.obtenir_piece_active()
        plateau = moteur.obtenir_plateau()
...
class PlateauDemoS:
    """Plateau simple pour la démonstration 6x6 avec pièce S."""
...
class MoteurDemo6x6S:
    """
    Moteur simple pour la démonstration 6x6 avec pièce S.
    Implémente le protocole MoteurJeu pour fonctionner avec les commandes.
    """
    
    def __init__(self):
        # Plateau 6x6 pour la démo
        self.plateau = PlateauDemoS(largeur=6, hauteur=6)
        
        # Créer la pièce S au centre
        fabrique = FabriquePieces()
        self.piece_active = fabrique.creer(TypePiece.S, x_pivot=2, y_pivot=2)
        
        # État du jeu
        self.en_pause = False
        self.afficher_menu = False
        
        print(f"🎮 Moteur démo S 6x6 initialisé")
        print(f"📍 Pièce S créée: {self.piece_active.positions}")
    
    def obtenir_piece_active(self) -> Piece:
        """Retourne la pièce actuellement contrôlée."""
        return self.piece_active
    
    def obtenir_plateau(self) -> PlateauDemoS:
        """Retourne le plateau de jeu."""
        return self.plateau
    
    def faire_descendre_piece(self) -> bool:
        """Fait descendre la pièce S d'une ligne si possible."""
        ...
        if self.plateau.peut_placer_piece(self.piece_active):
            print(f"⬇️ Descente S → {self.piece_active.positions}")
            return True
        else:
    ...
    def placer_piece_definitivement(self) -> None:
        """Place la pièce S définitivement sur le plateau."""
        # Placer la pièce sur le plateau
        self.plateau.placer_piece(self.piece_active)
        print(f"📍 Pièce S placée définitivement à: {self.piece_active.positions}")
    
    def generer_nouvelle_piece(self) -> None:
        """Génère une nouvelle pièce S (pour cette démo, on recrée une S)."""
        fabrique = FabriquePieces()
        self.piece_active = fabrique.creer(TypePiece.S, x_pivot=2, y_pivot=1)
        print(f"✨ Nouvelle pièce S générée: {self.piece_active.positions}")
    
    def chute_rapide_demo(self) -> bool:
        """Chute rapide modifiée pour la démo S - sans génération de nouvelle pièce."""
        ...
        if nb_lignes_descendues > 0:
            print(f"🚀 Chute rapide S: {nb_lignes_descendues} lignes → {self.piece_active.positions}")
...
class AffichageAvecGestionnaireS:
    """Affichage pour la démo S avec gestionnaire d'événements."""
    ...
    def __init__(self):
        ...
        pygame.display.set_caption("Demo 6x6 - Pièce S avec Gestionnaire d'Événements")
        
        # Couleurs
        self.noir = (0, 0, 0)
        self.gris = (64, 64, 64)
        self.blanc = (255, 255, 255)
        self.vert_s = (0, 255, 0)      # Vert pour pièce S
    ...
    def dessiner(self, moteur: MoteurDemo6x6S):
        ...
        for pos in moteur.piece_active.positions:
            ...
            rect = pygame.Rect(x, y, self.taille_cellule, self.taille_cellule)
            pygame.draw.rect(self.ecran, self.vert_s, rect)
            pygame.draw.rect(self.ecran, self.blanc, rect, 2)
        ...
        # Titre
        titre = "Demo 6x6 - Pièce S avec Gestionnaire d'Événements"
        ...
        instructions = [
            ...
            "ESPACE : Chute rapide | ENTRÉE : Placer pièce S",
...
class GestionnaireDemoPersonnaliseS(GestionnaireEvenements):
    """Gestionnaire d'événements personnalisé pour la démo S."""
    
    def _creer_commandes(self):
        """Crée le mapping avec nos commandes personnalisées pour la pièce S."""
        from src.domaine.services.commandes import (
            CommandeDescendre, CommandePause, CommandeAfficherMenu
        )
        
        return {
            ToucheClavier.GAUCHE: CommandeDeplacerGaucheDemoS(),           # Notre version corrigée pour S
            ToucheClavier.DROITE: CommandeDeplacerDroiteDemoS(),          # Notre version corrigée pour S
            ToucheClavier.ROTATION: CommandeTournerDemoS(),               # Notre version corrigée pour S
            ToucheClavier.CHUTE_RAPIDE: CommandeDescendre(),
            ToucheClavier.CHUTE_INSTANTANEE: CommandeChuteRapideDemoS(),  # Notre version modifiée pour S
            ToucheClavier.MENU: CommandeAfficherMenu(),
            ToucheClavier.PAUSE: CommandePause(),
        }


class DemoAvecGestionnaireS:
    """Démonstration principale utilisant le gestionnaire d'événements pour la pièce S."""
    
    def __init__(self):
        self.moteur = MoteurDemo6x6S()
        self.affichage = AffichageAvecGestionnaireS()
        self.gestionnaire = GestionnaireDemoPersonnaliseS()  # Notre gestionnaire personnalisé
        ...
        print("🎮 Démonstration S avec gestionnaire d'événements initialisée")
    ...
    def executer(self):
        ...
        print("\n=== DEMO 6x6 PIÈCE S AVEC GESTIONNAIRE D'ÉVÉNEMENTS ===")
            ...
            for event in pygame.event.get():
                ...
                # Touches spéciales pour la démo
                    ...
                    if event.key == pygame.K_RETURN:  # Entrée pour placer la pièce
                        ...
                        print("⏎ Pièce S placée manuellement avec Entrée")
        ...
        print("Démonstration S terminée.")


if __name__ == "__main__":
    demo = DemoAvecGestionnaireS()
    demo.executer()
