"""
Démonstration Plateau 6x6 avec Pièce L - Version avec Plateau Refactorisé
...
✅ Utilise le vrai Plateau(6, 6) au lieu de PlateauDemoL
...
class CommandeChuteRapideDemoL(Commande):
    """Commande de chute rapide pour la démo L."""
    
    def execute(self, moteur) -> bool:
        """Fait tomber la pièce L jusqu'en bas."""
        if hasattr(moteur, 'chute_rapide_demo'):
            return moteur.chute_rapide_demo()
        return False


class CommandeDeplacerGaucheDemoL(Commande):
    """Commande de déplacement gauche pour la démo L."""
    
    def execute(self, moteur) -> bool:
        """Déplace la pièce L vers la gauche si possible."""
        ...
        if plateau.peut_placer_piece(piece):
            print(f"⬅️ Gauche L → {piece.positions}")
            return True
        else:
...
class CommandeDeplacerDroiteDemoL(Commande):
    """Commande de déplacement droite pour la démo L."""
    
    def execute(self, moteur) -> bool:
        """Déplace la pièce L vers la droite si possible."""
        piece = moteur.obtenir_piece_active()
        plateau = moteur.obtenir_plateau()
        ...
        # Vérifier si le déplacement est valide
        if plateau.peut_placer_piece(piece):
            print(f"➡️ Droite L → {piece.positions}")
            return True
        else:
            # Annuler le déplacement
            piece.positions = positions_originales
...
class CommandeTournerDemoL(Commande):
    """Commande de rotation pour la démo L."""
    
    def execute(self, moteur) -> bool:
        """Fait tourner la pièce L si possible."""
        piece = moteur.obtenir_piece_active()
        plateau = moteur.obtenir_plateau()
        ...
        if plateau.peut_placer_piece(piece):
            print(f"🔄 Rotation L → {piece.positions}")
            return True
        else:
            # Annuler la rotation
            piece.positions = positions_originales
...
class MoteurDemo6x6LAvecPlateau:
    """
    Moteur pour la démonstration L utilisant le VRAI plateau refactorisé.
    ...
    def __init__(self):
        ...
        print(f"🎮 Moteur démo L 6x6 avec VRAI plateau initialisé")
    ...
    def faire_descendre_piece(self) -> bool:
        """Fait descendre la pièce L d'une ligne si possible."""
        ...
        if self.plateau.peut_placer_piece(self.piece_active):
            print(f"⬇️ Descente L → {self.piece_active.positions}")
            return True
        else:
    ...
    def placer_piece_definitivement(self) -> None:
        """Place la pièce L définitivement sur le plateau."""
        # Placer la pièce sur le vrai plateau
        self.plateau.placer_piece(self.piece_active)
        print(f"📍 Pièce L placée définitivement à: {self.piece_active.positions}")
    ...
    def chute_rapide_demo(self) -> bool:
        ...
        if nb_lignes_descendues > 0:
            print(f"🚀 Chute rapide L: {nb_lignes_descendues} lignes → {self.piece_active.positions}")
...
class AffichageAvecPlateauL:
    """Affichage pour la démo L avec le vrai plateau."""
    ...
    def __init__(self):
        ...
        pygame.display.set_caption("Demo 6x6 - Pièce L avec Plateau Refactorisé")
        
        # Couleurs
        self.noir = (0, 0, 0)
        self.gris = (64, 64, 64)
        self.blanc = (255, 255, 255)
        self.orange = (255, 165, 0)       # Orange pour pièce L active
    ...
    def dessiner(self, moteur: MoteurDemo6x6LAvecPlateau):
        ...
        # Titre
        titre = "Demo 6x6 - Pièce L avec Plateau Refactorisé"
...
class GestionnaireDemoLAvecPlateau(GestionnaireEvenements):
    """Gestionnaire d'événements pour la démo L avec vrai plateau."""
    
    def _creer_commandes(self):
        """Crée le mapping avec nos commandes pour la pièce L."""
        from src.domaine.services.commandes import (
            CommandeDescendre, CommandePause, CommandeAfficherMenu
        )
        
        return {
            ToucheClavier.GAUCHE: CommandeDeplacerGaucheDemoL(),
            ToucheClavier.DROITE: CommandeDeplacerDroiteDemoL(),
            ToucheClavier.ROTATION: CommandeTournerDemoL(),
            ToucheClavier.CHUTE_RAPIDE: CommandeDescendre(),
            ToucheClavier.CHUTE_INSTANTANEE: CommandeChuteRapideDemoL(),
            ToucheClavier.MENU: CommandeAfficherMenu(),
            ToucheClavier.PAUSE: CommandePause(),
        }


class DemoLAvecPlateauRefactorise:
    """Démonstration L utilisant le vrai plateau refactorisé."""
    
    def __init__(self):
        self.moteur = MoteurDemo6x6LAvecPlateau()
        self.affichage = AffichageAvecPlateauL()
        self.gestionnaire = GestionnaireDemoLAvecPlateau()
        ...
        print("🚀 Démonstration L avec plateau REFACTORISÉ initialisée !")
    ...
    def executer(self):
        ...
        print("\n=== DEMO L AVEC PLATEAU REFACTORISÉ ===")
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if event.key == pygame.K_RETURN:
                        ...
                        print("⏎ Pièce L placée manuellement")
        ...
        pygame.quit()
        print("\n✨ Démonstration L avec plateau refactorisé terminée.")
        print("🎯 Vous avez vu les bénéfices de la refactorisation en action !")


if __name__ == "__main__":
    print("🚀 Démarrage de la démonstration L avec plateau REFACTORISÉ...")
    print("🎯 Cette version montre les bénéfices concrets de la refactorisation !")
