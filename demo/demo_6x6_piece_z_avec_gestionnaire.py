"""
Démonstration Plateau 6x6 avec Pièce Z - Version avec Gestionnaire d'Événements

Cette version utilise le vrai système de commandes et gestionnaire d'événements
du projet Tetris, montrant l'intégration complète de l'architecture avec la pièce Z.
...
class CommandeChuteRapideDemoZ(Commande):
    """Commande de chute rapide modifiée pour la démo Z - sans génération de nouvelle pièce."""
    
    def execute(self, moteur) -> bool:
        """Fait tomber la pièce Z jusqu'en bas sans la remplacer."""
        if hasattr(moteur, 'chute_rapide_demo'):
            return moteur.chute_rapide_demo()
        return False


class CommandePlacerPieceDemoZ(Commande):
    """Commande pour placer une pièce Z définitivement sur le plateau."""
    
    def execute(self, moteur) -> bool:
        """Place la pièce Z sur le plateau et génère une nouvelle pièce Z."""
...
class CommandeDeplacerGaucheDemoZ(Commande):
    """Commande de déplacement gauche corrigée pour la démo Z."""
...
class CommandeDeplacerDroiteDemoZ(Commande):
    """Commande de déplacement droite corrigée pour la démo Z."""
...
class CommandeTournerDemoZ(Commande):
    """Commande de rotation corrigée pour la démo Z."""
...
class PlateauDemoZ:
    """Plateau simple pour la démonstration 6x6 avec pièce Z."""
...
class MoteurDemo6x6Z:
    """
    Moteur simple pour la démonstration 6x6 avec pièce Z.
    Implémente le protocole MoteurJeu pour fonctionner avec les commandes.
    """
    
    def __init__(self):
        # Plateau 6x6 pour la démo
        self.plateau = PlateauDemoZ(largeur=6, hauteur=6)
        ...
        print(f"🎮 Moteur démo Z 6x6 initialisé")
    ...
    def obtenir_plateau(self) -> PlateauDemoZ:
    ...
    def generer_nouvelle_piece(self) -> None:
        """Génère une nouvelle pièce Z (pour cette démo, on recrée une Z)."""
        fabrique = FabriquePieces()
    ...
    def chute_rapide_demo(self) -> bool:
        """Chute rapide modifiée pour la démo Z - sans génération de nouvelle pièce."""
...
class AffichageAvecGestionnaireZ:
    """Affichage pour la démo Z avec gestionnaire d'événements."""
    ...
    def __init__(self):
        ...
        pygame.display.set_caption("Demo 6x6 - Pièce Z avec Gestionnaire d'Événements")
        
        # Couleurs
        self.noir = (0, 0, 0)
        self.gris = (64, 64, 64)
        self.blanc = (255, 255, 255)
        self.rouge_z = (255, 0, 0)     # Rouge pour pièce Z
    ...
    def dessiner(self, moteur: MoteurDemo6x6Z):
        ...
        # Pièces du plateau (si il y en a) - en magenta pour les distinguer de la pièce Z active
        for position in moteur.plateau.obtenir_positions_occupees():
            ...
            rect = pygame.Rect(x, y, self.taille_cellule, self.taille_cellule)
            pygame.draw.rect(self.ecran, (255, 0, 255), rect)  # Magenta pour pièces placées
        ...
        for pos in moteur.piece_active.positions:
            ...
            rect = pygame.Rect(x, y, self.taille_cellule, self.taille_cellule)
            pygame.draw.rect(self.ecran, self.rouge_z, rect)
            pygame.draw.rect(self.ecran, self.blanc, rect, 2)
        ...
        # Titre
        titre = "Demo 6x6 - Pièce Z avec Gestionnaire d'Événements"
        ...
        instructions = [
            ...
            "ESPACE : Chute rapide | ENTRÉE : Placer pièce Z",
...
class GestionnaireDemoPersonnaliseZ(GestionnaireEvenements):
    """Gestionnaire d'événements personnalisé pour la démo Z."""
    
    def _creer_commandes(self):
        """Crée le mapping avec nos commandes personnalisées pour la pièce Z."""
        from src.domaine.services.commandes import (
            CommandeDescendre, CommandePause, CommandeAfficherMenu
        )
        
        return {
            ToucheClavier.GAUCHE: CommandeDeplacerGaucheDemoZ(),           # Notre version corrigée pour Z
            ToucheClavier.DROITE: CommandeDeplacerDroiteDemoZ(),          # Notre version corrigée pour Z
            ToucheClavier.ROTATION: CommandeTournerDemoZ(),               # Notre version corrigée pour Z
            ToucheClavier.CHUTE_RAPIDE: CommandeDescendre(),
            ToucheClavier.CHUTE_INSTANTANEE: CommandeChuteRapideDemoZ(),  # Notre version modifiée pour Z
            ToucheClavier.MENU: CommandeAfficherMenu(),
            ToucheClavier.PAUSE: CommandePause(),
        }


class DemoAvecGestionnaireZ:
    """Démonstration principale utilisant le gestionnaire d'événements pour la pièce Z."""
    
    def __init__(self):
        self.moteur = MoteurDemo6x6Z()
        self.affichage = AffichageAvecGestionnaireZ()
        self.gestionnaire = GestionnaireDemoPersonnaliseZ()  # Notre gestionnaire personnalisé
        ...
        print("🎮 Démonstration Z avec gestionnaire d'événements initialisée")
    ...
    def executer(self):
        ...
        print("\n=== DEMO 6x6 PIÈCE Z AVEC GESTIONNAIRE D'ÉVÉNEMENTS ===")
            ...
            for event in pygame.event.get():
                ...
                # Touches spéciales pour la démo
                    ...
                    if event.key == pygame.K_RETURN:  # Entrée pour placer la pièce
                        ...
                        print("⏎ Pièce Z placée manuellement avec Entrée")
        ...
        print("Démonstration Z terminée.")


if __name__ == "__main__":
    demo = DemoAvecGestionnaireZ()
    demo.executer()
